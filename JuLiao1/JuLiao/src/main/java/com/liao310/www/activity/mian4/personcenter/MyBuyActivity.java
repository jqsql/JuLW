...
public class MyBuyActivity  extends BaseActivity {
	...
	private MyBuyActivity _this;
	private ImageView back;
	private TextView title;

	private RefreshListView refreshListView;
	private ArticleListAdapter myBuyAdapter = null;
	...
	private void initView() {
		...
		myBuyAdapter = new ArticleListAdapter(_this,3,2);
		refreshListView.setAdapter(myBuyAdapter);
	...
	private void getMyBuy(final boolean isMore) {
		ServicePersonal.getInstance().getMyBuy(_this,more,
			...
			public void onFailure(ErrorMsg errorMessage) {
				...
				if(myBuyAdapter!=null){
					myBuyAdapter.notifyDataSetChanged();// 重新刷新数据
				}
